...
class RLTrainer:
    ...
    def __call__(self):
        ...
        if self._dump_data:
            ...
            samples["sampled_act"] = np.zeros_like(samples["act"])
            ...
            for batch_idx in range(n_batch):
                ...
                states = torch.from_numpy(samples["obs"][start_idx:last_idx]).to(self.device)
                ...
                sample_actions, _ = self._policy.actor(states)
                samples["sampled_act"][start_idx:last_idx] = sample_actions.to('cpu').detach().numpy().copy()
            ...
            torch.save(self._policy.qf1.state_dict(),
                       os.path.join(self._output_dir,
                                    f'critic_q_final_{self._policy.exp_name}.pth'))
